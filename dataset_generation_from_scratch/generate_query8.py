...
result_unstr = spark.sql(""" select distinct hadm_id,upper(arg2_value) as drug,arg1_value as form from i2b2_lookup_extract  where upper(type)='FORM-DRUG'   
""")


def execute_unstructured_query8():	
    ...
    for nextValue in newdict:
        ...
        naturalQuestion = 'What is the form of ' + nextValue[7:len(nextValue)] + ' prescribed to the patient with admission id ' + nextValue[0:6]

        query ='SELECT PRESCRIPTIONS.form_unit_disp FROM PRESCRIPTIONS WHERE PRESCRIPTIONS.hadm_id = '+nextValue[0:6]+' and PRESCRIPTIONS.drug = "'+nextValue[7:len(nextValue)] +'"'
...
unstr_df = execute_unstructured_query8()
...
result_str = spark.sql(""" select hadm_id,drug,form_unit_disp from (
        select distinct ps.hadm_id as hadm_id,upper(ps.drug) as drug,ps.form_unit_disp from prescriptions ps inner join i2b2_lookup i2b2 on ps.hadm_id=i2b2.hadm_id and upper(ps.drug) = upper(i2b2.value)) where form_unit_disp is not null
            
""")

def execute_structured_query8():	
...
str_df = execute_structured_query8()
...
print(df)
df.to_excel("non_paraphrased_queries/query8.xlsx", index = False,  header=True)
