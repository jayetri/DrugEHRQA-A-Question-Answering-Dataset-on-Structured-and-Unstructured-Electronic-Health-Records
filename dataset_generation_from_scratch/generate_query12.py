...
result_unstr = spark.sql(""" select distinct hadm_id,upper(arg2_value) as drug,arg1_value as dosage from i2b2_lookup_extract  where upper(type)='DOSAGE-DRUG'   
""")


def execute_unstructured_query12():	
    ...
    for nextValue in newdict:
        ...
        naturalQuestion = 'What is the dosage of ' + nextValue[7:len(nextValue)] + ' prescribed to the patient with admission id ' + nextValue[0:6]
        
        query ='SELECT PRESCRIPTIONS.dose_val_rx, PRESCRIPTIONS.dose_unit_rx FROM PRESCRIPTIONS WHERE PRESCRIPTIONS.hadm_id = '+nextValue[0:6]+ ' and PRESCRIPTIONS.drug = "'+ nextValue[7:len(nextValue)] + '"'
...
unstr_df = execute_unstructured_query12()
...
result_str = spark.sql("""
        select distinct ps.hadm_id as hadm_id,upper(ps.drug)as drug,ps.dose_val_rx,ps.dose_unit_rx from prescriptions ps inner join i2b2_lookup i2b2 on ps.hadm_id=i2b2.hadm_id and upper(ps.drug) = upper(i2b2.value)           
""")



def execute_structured_query12():	
    ...
    for row in result_str.rdd.collect():
        ...
        if(row[2] and row[3]):
            if((row[0]+"|"+row[1]) not in newdict):
                newdict[row[0]+"|"+row[1]] = row[2] + row[3]
            else:
                newdict[row[0]+"|"+row[1]] = str(newdict[row[0]+"|"+row[1]])+","+row[2]+row[3]
    for nextValue in newdict:
...
str_df = execute_structured_query12()
...
print(df)
df.to_excel("non_paraphrased_queries/query12.xlsx", index = False,  header=True)
